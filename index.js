...
module.exports = function AutoGuildquest(mod) {
  ...
  mod.hook("S_FIELD_EVENT_PROGRESS_INFO", 1, () => {
    if (settings.Guardian)
      setTimeout(completeGuardian, 2000 + Math.random() * 250);
  });
  ...
  function report() {
    if (daily < 16)
      sendMessage(niceName + "Daily Vanguard Requests completed: " + daily);
    else
      sendMessage(
        niceName + "You have completed all 16 Vanguard Requests today."
      );
  }
  ...
  function dailycredit() {
    if (settings.Daily) {
      ...
      !_
        ? sendMessage("Unmapped protocol packet <C_REQUEST_RECV_DAILY_TOKEN>.")
        : null;
    }
  }
  function GetQuestsInfo(questEvent) {
    for (let questIndex in questEvent) {
      if ([1, 2].includes(questEvent[questIndex]["status"])) {
        ...
        let qStatus = `${
          questEvent[questIndex]["status"] == 1
            ? "[ACTIVE]".clr("f1ef48")
            : "[COMPLETE]".clr("3fce29")
        }`;
        let qTime = new Date(1000 * questEvent[questIndex]["timeRemaining"])
          .toISOString()
          .substr(11, 8);
        sendMessage(
          `${qStatus} ${Quests[qName].clr("0cccd6")} ${qSize.clr(
            "0c95d4"
          )} Time left: ${qTime.clr("db3dce")}`
        );
      } else {
        continue;
      }
  ...
  mod.hook("S_CHAT", 3, (event) => {
    ...
    if (event.channel === 2 && message === "@quest") {
      quest();
      MSG.guild("Try accept guild quest");
    } else if (event.channel === 2 && message === "<FONT>@quest</FONT>") {
      quest();
      MSG.guild("Try accept guild quest");
    }
  });
  ...
  command.add("auto", {
    AC: () => {
      ...
      sendMessage(
        "AutoQuests-Mod is now : " + (settings.enabled ? "On" : "Off")
      );
    },
    VG: () => {
    ...
    GL: () => {
      ...
      sendMessage(
        "Auto-Guardian-Legion: " + (settings.Guardian ? "On" : "Off")
      );
    },
    DC: () => {
